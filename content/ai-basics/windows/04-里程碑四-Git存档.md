...
```text
在"占占AI小白入门"文件夹中创建一个叫 git-practice 的文件夹，并在里面创建 intro.txt，写入"这是我的第一个 Git 项目"。不要执行 Git 命令。
```
...
> 📸 操作截图尚未补齐：git status 输出，Untracked files 区圈红
🔍 **刚才发生了什么**
`git init` 在项目里创建了隐藏的 `.git` 文件夹（想看的话：文件资源管理器顶部 查看 → 显示 → 勾选「隐藏的项目」）。`Untracked` 的意思是「Git 发现了这个文件，但还没开始记录它」。另外记住：**`git status` 永远安全**——它只汇报状态、不改任何东西，可以随便运行。
...
> 📸 操作截图尚未补齐：commit + status + log 三连输出
🔍 **刚才发生了什么**
...
**下一站 →** [[ai-basics/windows/05-里程碑五-GitHub协作|里程碑五：用 GitHub 保存项目和参与协作]]
